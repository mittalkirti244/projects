...
function addField(sectionType, appSchema, field, targetID, key, section, version, withActions = true) {
    ...
    if (!field || !field.$Type) {
        ...
        const fieldsDefinition = __1.prepareRef(`${sectionType}${ux_specification_types_1.DefinitionName.Fields}<${targetID}>`);
    ...
    //Add tags
    appSchema.definitions[fieldDefinition][ux_specification_types_1.SchemaTag.annotationPath] = `/${section.entityType.fullyQualifiedName}/${field.fullyQualifiedName.split(section.entityType.fullyQualifiedName)[1]}`;
    appSchema.definitions[fieldDefinition][ux_specification_types_1.SchemaTag.annotationType] = field.$Type;
    ...
    if (field.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAction" /* DataFieldForAction */ && appSchema.definitions?.['FormAction']) {
        appSchema.definitions[fieldDefinition].properties = appSchema.definitions[ux_specification_types_1.DefinitionName.FormAction].properties;
    }
}
/**
...
function handleForm(sectionType, section, appSchemaSection, appSchema, version, withActions = true) {
    ...
    const formDefinition = __1.prepareRef(`${sectionType}${ux_specification_types_1.DefinitionName.Form}<${targetID}>`);
    const fieldsDefinition = __1.prepareRef(`${sectionType}${ux_specification_types_1.DefinitionName.Fields}<${targetID}>`);
    const actionsDefinition = __1.prepareRef(`${sectionType}${ux_specification_types_1.DefinitionName.Actions}<${targetID}>`);
    // Form definition
...
function addFooterActions(appSchema, generateParameters) {
    ...
    const alias = __1.findAlias(ux_specification_types_1.UIVOCABULARY, generateParameters.serviceAVT);
    ...
    if (actionAnnotation) {
        ...
        footerActions[ux_specification_types_1.SchemaTag.annotationPath] = `/${entityType.fullyQualifiedName}/@${"com.sap.vocabularies.UI.v1.Identification" /* Identification */}`;
    }
...
function addHeaderActions(appSchema, entityType, oDataServiceAVT, addHeaderActionCallBack) {
    appSchema.definitions[ux_specification_types_1.DefinitionName.ObjectPageHeader].properties[ux_specification_types_1.PropertyName.actions] = {
        $ref: `${__1.DEFINITION_LINK_PREFIX}${ux_specification_types_1.DefinitionName.HeaderActions}`
    };
    const headerActions = (appSchema.definitions[ux_specification_types_1.DefinitionName.HeaderActions] = {
        type: 'object',
        properties: {},
        description: 'Actions',
        additionalProperties: false
    ...
    const alias = __1.findAlias(ux_specification_types_1.UIVOCABULARY, oDataServiceAVT);
    ...
    if (identification) {
        ...
        headerActions[ux_specification_types_1.SchemaTag.annotationPath] = `/${entityType.fullyQualifiedName}/@${"com.sap.vocabularies.UI.v1.Identification" /* Identification */}`;
    }
...
function addCommonHeaderSchema(appSchema, entityType) {
    ...
    appSchema.definitions[ux_specification_types_1.DefinitionName.ObjectPageHeader][ux_specification_types_1.SchemaTag.annotationPath] = `/${entityType.fullyQualifiedName}/@${"com.sap.vocabularies.UI.v1.HeaderInfo" /* HeaderInfo */}`;
}
